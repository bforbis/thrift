...
import { InputBufferUnderrunError, THTTPException } from '../exception';

export { default as createHttpClient } from './create_client';
 ...
 *        https: true
 *     };
 *     const con = thrift.createHttpConnection("thrift.example.com", 9090, options);
 *     const client = thrift.createHttpClient(myService, connection);
...
export class HttpConnection<
  TTransport extends TAbstractTransport,
  TProtocol extends TAbstractProtocol
> extends EventEmitter implements thrift.TConnection {
  ...
  public nodeOptions: { [key: string]: any };
  public seqId2Service: { [key: number]: string };
  public responseCallback: (response: http.IncomingMessage) => void;

  // set in create_client.ts
  public client?: thrift.ServiceClient;
   ...
   */
  public constructor(options?: ConnectionOptions<TTransport, TProtocol>) {
    ...
    for (const attrname in this.options.nodeOptions) {
    ...
    function decodeCallback(transport_with_data: TTransport): void {
      const proto = new self.protocol(transport_with_data);
      try {
        while (true) {
    ...
    //Response handler
    //////////////////////////////////////////////////
    this.responseCallback = function(response: http.IncomingMessage): void {
      const data: Buffer[] = [];
      let dataLen = 0;
      ...
      response.on('data', function(chunk: Buffer | string | Uint8Array) {
      ...
      response.on('end', function() {
        ...
        self.transport.receiver(decodeCallback, 0)(buf);
      });
    };
  }
...
): HttpConnection<TTransport, TProtocol> {
  options.host = host;
  options.port = port || 80;
  return new HttpConnection(options);
}

export function createHttpUDSConnection<
  TTransport extends TAbstractTransport,
  TProtocol extends TAbstractProtocol
>(
  path: string,
