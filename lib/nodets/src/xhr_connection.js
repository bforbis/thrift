...
let TBufferedTransport = require('./buffered_transport');
let TJSONProtocol = require('./json_protocol');
let InputBufferUnderrunError = require('./input_buffer_underrun_error');

let createClient = require('./client/create_client');
...
function XHRConnection(host, port, options) {
  ...
  let prefix = options.https ? 'https://' : 'http://';
...
XHRConnection.prototype.__decodeCallback = function(transport_with_data) {
  let proto = new this.protocol(transport_with_data);
  try {
    while (true) {
      ...
      //  operation soon.
      //  --ra
      let service_name = this.seqId2Service[header.rseqid];
      if (service_name) {
      ...
      client._reqs[dummy_seqid] = function(err, success) {
        ...
        let clientCallback = client._reqs[header.rseqid];
...
XHRConnection.prototype.read = function(len) {
  ...
  let ret = this.read_buf.substr(this.rpos, give);
  this.rpos += give;
